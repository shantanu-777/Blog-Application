...
public class UserService : IUserService
{
    ...
    public async Task<bool> IsFollowingAsync(string followerId, string followingId)
    {
        try
        {
            var response = await _httpClient.GetAsync($"api/user/{followingId}/isfollowing/{followerId}");
            return response.IsSuccessStatusCode;
        }
        catch (Exception)
        {
    ...
    public async Task<bool> FollowUserAsync(string currentUserId, string userIdToFollow)
    {
        return await FollowUserAsync(userIdToFollow);
    }

    public async Task<bool> UnfollowUserAsync(string currentUserId, string userIdToUnfollow)
    {
        return await UnfollowUserAsync(userIdToUnfollow);
    }
}
